...
for K=1:2
  ...
  sa=tools.math.stat.rstd(dA(cl)); sd=tools.math.stat.rstd(dD(cl)); th=linspace(0,2*pi,200);
  ...
  title(sprintf(['%s: target \\Delta\\mu = %+.2f / %+.2f, Gaia G=%.1f RUWE=%.2f\n', ...
      'population rstd %.2f / %.2f; target at %.0f%% of %d clean stars, %.0f%% of %d within 0.5 mag'], ...
  ...
  fprintf('%s: target dmu %+.3f / %+.3f, Gaia err %.3f / %.3f, internal %.3f / %.3f -> %.1f / %.1f sigma; G %.2f RUWE %.2f; clean %d; pop rstd %.3f/%.3f; pct %.0f%% (all) %.0f%% (within 0.5 mag, n=%d)\n', ...
...
title(TL,'v8: our absolute proper motion minus Gaia DR3, per star (grey circles: 1, 2, 3 robust sigma of the clean population)','FontWeight','bold','FontSize',14);
exportgraphics(Fh,[Doc 'qa_v8_pmdiff_vs_gaia.png'],'Resolution',150); close(Fh);
