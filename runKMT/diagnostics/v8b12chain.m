...
Sys = sprintf('/home/ocs/KMTdata/Results/v8b12/SysCor_%s.mat', F);
Fin = sprintf('/home/ocs/KMTdata/Results/v8b12/IFfinal_%s.mat', F);
...
T0=tic;
[~,~,~,I2]=ml.scripts.runIterDetrendMSc(MS,'NColourBins',12,'Verbosity',0);
...
[IFsys,~,IFsysB,Info]=ml.scripts.runIterDetrendMSc(MS, ...
    'SysRemCorrection',Pre,'PixPhase',true,'NColourBins',12, ...
...
if isempty(Ie)
    error('v8chain:TargetMissing','the target (MSc %d) is not in the solution', P.TargetMsc);
end
...
Config=struct('Input',In,'Field',F,'Note','v8b12: v8 with 12 colour bins', ...
