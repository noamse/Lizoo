...
Doc='/home/ocs/matlab/Lizoo/doc/'; AxN='XY'; SidMonth=27.321661;
...
for K=1:2
  ...
  for q=1:numel(Sel.tags)
     if isfinite(Sel.(f)(q)), List(end+1,:)={Sel.(f)(q), Sel.tags{q}, 'calibration star'}; end
  end
  for L=1:size(List,1)
    ...
    title(TL,sprintf(['v13  %s  %s  —  I = %.2f%s, at (%.0f,%.0f)   |   ', ...
    ...
    exportgraphics(Fh,sprintf('%sreport_v13_motion_%s_%s.png',Doc,f,tag),'Resolution',150);
    close(Fh);
    fprintf('%s %-9s I %.2f d %5.1f | PM %+6.2f/%+6.2f | factor %.2f / %.2f\n', f, tag, I(Ii), dist, PMabs(1,Ii), PMabs(2,Ii), Fe);
  end
  clear IF V
end
fprintf('V13FIGS DONE\n');
