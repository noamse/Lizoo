function [AllSI,JD, MSc, AllShifted] = KMT_pipelineI(RawImageList, Args)
    %
    % Example: D.loadCalib();
    ...
    arguments
        ...
        Args.TempName                      = '/bigdata3/projects/KMTdata/Images/260058/KB260058_20*_CTIO_I_BLG41/RAW/*I*.fits';  %'/bigdata3/projects/KMTdata/Images/260058/2026_CTIO_I/BLG01/*I*.fits'; %/home/yossishv/KMTdata/ob260058_kmtc/2026/BLG01/subkmtcBLG01I202607*fits';%'subkmtcBLG01I202607*fits';'2023/BLG01/subkmt*I*.fits';%
        ...
        Args.RA                            = celestial.coo.convertdms('17:52:38.09','gH','d');   % deg
        Args.Dec                           = celestial.coo.convertdms('-31:47:36.1','gD','d');   % deg
        ...
        Args.PopPSFArgs                     = {'WingsThreshold',0.15,'CropByQuantile',false, 'SuppressWidth',3, 'SmoothWings',false,'WingsMethod','empirical'};
        Args.Threshold                     = [100 50 20]; %[500 50 20 4];
        ...
        Args.SubSizeXY                     = []; % tested using: RR=imUtil.filter.fft_size_timing([Size Size],false,10000);
        Args.EdgesCCDSEC                   = [];
    ...
    % filter bad images
        ...
        NotConstantImage = true(Nim,1);

        % filter images with abnormal median
        MedianIm = imProc.stat.median(AI);
        FlagMedian = MedianIm>50 & MedianIm<20000;


        % filter empty images
        NotEmptyImage = ~AI.isemptyImage;
        GoodGlobalBack = imProc.quality.backgroundLevel(AI, 'UseMex',Args.UseMex,'ThresholdBack',Args.ThresholdBack);
    ...
    % Shift images and catalogs based on cross correlation into the frame
    % of image index Iref
    Iref = 500;
    N = numel(AllSI);
    ShiftX = nan(N,1);
    ShiftY = nan(N,1);
    NormPeakCorr = nan(N,1);
    for I=1:1:N
        ColX = AllSI(I).CatData.colname2ind('X');
        ColY = AllSI(I).CatData.colname2ind('Y');

        % clean catalog
        Nsrc = AllSI(I).sizeCatalog;
        for Isrc=1:1:Nsrc
            Dist = tools.math.geometry.plane_dist(AllSI(I).CatData.Catalog(Isrc,ColX), AllSI(I).CatData.Catalog(Isrc,ColY), AllSI(I).CatData.Catalog(:,ColX), AllSI(I).CatData.Catalog(:,ColY));
            Inear = find(Dist<0.5);
            if Inear==1
                % skip
            else
                if AllSI(I).CatData.Table.SN(Isrc)>=AllSI(I).CatData.Table.SN(Inear)
                    Iremove = setdiff(Inear, Isrc);
                    AllSI(I).CatData.Catalog(Iremove,:) = NaN;
                end
            end
        end
        IsN = isnan(AllSI(I).CatData.Catalog(:,ColX));
        AllSI(I).CatData.Catalog = AllSI(I).CatData.Catalog(~IsN,:);
                


        [~,Info]=imUtil.filter.shiftFromXCorr(AllSI(I).Image, AllSI(Iref).Image);
        ShiftX(I) = Info.ShiftXY(1);
        ShiftY(I) = Info.ShiftXY(2);
        NormPeakCorr(I) = Info.NormPeakCorr;

        
        %Dist = tools.math.geometry.plane_dist(139,179, AllSI(I).CatData.Catalog(:,ColX), AllSI(I).CatData.Catalog(:,ColY));
        %Idist = find(Dist<1);
        %Idist


        Shifted=imUtil.trans.mex.shift_lanczos3(AllSI(I).Image,-ShiftX(I),-ShiftY(I));

        AllShifted(I) = AllSI(I).copy;
        AllShifted(I).Image = Shifted;
        
        
        AllShifted(I).CatData.Catalog(:,ColX) = AllShifted(I).CatData.Catalog(:,ColX) - ShiftX(I);
        AllShifted(I).CatData.Catalog(:,ColY) = AllShifted(I).CatData.Catalog(:,ColY) - ShiftY(I);

        AllShifted(I).CatData.sortrows('Y');
        AllSI(I).CatData.sortrows('Y');
    ...
    load('OB160058.mat');% ./OGLEdata/OB260058/OB160058.mat
    OB160058.cast('single');
    OgleRefCat = OB160058.CatData.Table;
    OgleRefCat.Xogle = OgleRefCat.X;
    ...
    OgleRefCat = OgleRefCat(FlagMag,:);
    OgleRefCat       = sortrows(OgleRefCat, 'Y');


    SearchRadius = 1.5;
    [Ind1, Dist1, Nmatch1, Ind2, Dist2, Nmatch2] = imUtil.match.mex.matchCatalogsXY(MS.SrcData.X(:), MS.SrcData.Y(:), OgleRefCat.X, OgleRefCat.Y, SearchRadius);

    NN = ~isnan(Ind1);
    MS.SrcData.I_ogle = nan(1,MS.Nsrc);
    MS.SrcData.V_ogle = nan(1,MS.Nsrc);
    ...
    % object can be detrended later without the images being available
    % (see ml.util.mmsFromMatchedSources). The vectors are stored at their
    % full length together with the selection flags, so that they can be
    % aligned whether or not the selection above reduced the object.
    MSc.UserData = struct('ShiftX',        ShiftX(:), ...
                          'ShiftY',        ShiftY(:), ...
                          'NormPeakCorr',  NormPeakCorr(:), ...
                          'FlagGoodEpoch', FlagGoodEpoch(:), ...
                          'FlagGoodSrc',   FlagGoodSrc(:), ...
                          'Iref',          Iref, ...
                          'RA',            Args.RA, ...
                          'Dec',           Args.Dec, ...
    ...
    SearchRadius = 3.0;
    [~, ~, Nmatch1] = imUtil.match.mex.matchCatalogsXY(MS.SrcData.X(:), MS.SrcData.Y(:), OgleRefCat.X, OgleRefCat.Y, SearchRadius);


    SrcInfo.Nd10 = nan(MS.Nsrc,1);
    SrcInfo.Nd1  = nan(MS.Nsrc,1);
    SrcInfo.Is1  = nan(MS.Nsrc,1);
    SrcInfo.Dist1  = nan(MS.Nsrc,1);
    SrcInfo.I_ogle  = nan(MS.Nsrc,1);
    SrcInfo.V_ogle  = nan(MS.Nsrc,1);
    SrcInfo.MaxDeltaMag10  = -inf(MS.Nsrc,1);

    for Isrc=1:1:MS.Nsrc
        Dist = tools.math.geometry.plane_dist(MS.SrcData.X(Isrc), MS.SrcData.Y(Isrc), OgleRefCat.X, OgleRefCat.Y);
        SrcInfo.Nd10(Isrc) = sum(Dist<5);
        SrcInfo.Nd1(Isrc) = sum(Dist<1);
        if SrcInfo.Nd1(Isrc)==1
            SrcInfo.Is1(Isrc) = 1;
            [IndDist1] = find(Dist<1);
            SrcInfo.Dist1(Isrc)     = Dist(IndDist1);
            SrcInfo.IndOgle(Isrc)   = IndDist1;
            SrcInfo.I_ogle(Isrc)    = OgleRefCat.I(IndDist1);
            SrcInfo.V_ogle(Isrc)    = OgleRefCat.V(IndDist1);

            IndDist10 = find(Dist<5 & Dist>=1);
            if numel(IndDist10)>0
                % negative is good...
                SrcInfo.MaxDeltaMag10(Isrc) = max(SrcInfo.I_ogle(Isrc) - OgleRefCat.I(IndDist10));
            else
                SrcInfo.MaxDeltaMag10(Isrc) = -Inf;
            end

        end
    end

    FlagGoodSrc = SrcInfo.MaxDeltaMag10<-0.5 & SrcInfo.Is1==1;
